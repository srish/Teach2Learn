...
function () {
    ...
    return this.opts.controlsPosition.indexOf("bottom") > -1 ? c += '<div class="drawing-board-controls"></div>' : c = '<div class="drawing-board-controls"></div>' + c, this.$el.addClass("drawing-board").append(c), this.dom = {
        ...
        history: !1,
        background: !1
    }), this.initControls(), this.resize(), this.initImageUploader(), this.reset({
        webStorage: !1,
        history: !0,
        background: !0
    }), this.restoreWebStorage(), this.initDropEvents(), this.initDrawEvents(), void 0) : (this.opts.errorMessage && this.$el.html(this.opts.errorMessage), !1)
    ...
    reset: function (a) {
        a = $.extend({
            ...
            history: !0,
            background: !1
        }, a), this.setMode("pencil"), a.background && this.resetBackground(this.opts.background, !1), a.color && this.setColor(a.color), a.size && (this.ctx.lineWidth = a.size), this.ctx.lineCap = "round", this.ctx.lineJoin = "round", a.webStorage && this.saveWebStorage(), a.history && this.saveHistory(), this.blankCanvas = this.getImg(), this.ev.trigger("board:reset", a)
    },
    resetBackground: function (a, b) {
        a = a || this.opts.background, b = "undefined" != typeof b ? b : !0;
        var c = DrawingBoard.Utils.isColor(a),
            d = this.getMode();
        this.setMode("pencil"), this.ctx.clearRect(0, 0, this.ctx.canvas.width, this.ctx.canvas.width), c ? (this.ctx.fillStyle = a, this.ctx.fillRect(0, 0, this.ctx.canvas.width, this.ctx.canvas.height)) : a && this.setImg(a), this.setMode(d), b && this.saveHistory()
    },
    resize: function () {
    ...
    saveHistory: function () {
        for (; this.history.values.length > 30;) this.history.values.shift(), this.history.position--;
        0 !== this.history.position && this.history.position < this.history.values.length ? (this.history.values = this.history.values.slice(0, this.history.position), this.history.position++) : this.history.position = this.history.values.length + 1, this.history.values.push(this.getImg()), this.ev.trigger("historyNavigation", this.history.position)
    },
    _goThroughHistory: function (a) {
        if (!(a && this.history.position == this.history.values.length || !a && 1 == this.history.position)) {
            var b = a ? this.history.position + 1 : this.history.position - 1;
            this.history.values.length && void 0 !== this.history.values[b - 1] && (this.history.position = b, this.setImg(this.history.values[b - 1])), this.ev.trigger("historyNavigation", b), this.saveWebStorage()
        }
    },
    ...
    setImg: function (a) {
        ...
        c.onload = function () {
            b.globalCompositeOperation = "source-over", b.clearRect(0, 0, b.canvas.width, b.canvas.width), b.drawImage(c, 0, 0), b.globalCompositeOperation = d
        }, c.src = a
    },
    getImg: function () {
    ...
    initDropEvents: function () {
        ...
        }), this.dom.$canvas.on("drop", $.proxy(this._onCanvasDrop, this)), void 0) : !1
    },
    _onCanvasDrop: function (a) {
        ...
        c.readAsDataURL(b[0]), c.onload = $.proxy(function (a) {
            this.setImg(a.target.result), this.ev.trigger("board:imageDropped", a.target.result), this.ev.trigger("board:userAction"), this.saveHistory()
        }, this)
    },
    ...
    fill: function (a) {
        if (this.getImg() === this.blankCanvas) return this.ctx.clearRect(0, 0, this.ctx.canvas.width, this.ctx.canvas.width), this.ctx.fillStyle = this.color, this.ctx.fillRect(0, 0, this.ctx.canvas.width, this.ctx.canvas.height), void 0;
        var b = this.ctx.getImageData(0, 0, this.canvas.width, this.canvas.height),
            c = 0,
            d = 1,
            e = 2,
            f = 3,
            g = this.ctx.strokeStyle,
            h = parseInt(g.substr(1, 2), 16),
            i = parseInt(g.substr(3, 2), 16),
            j = parseInt(g.substr(5, 2), 16),
            k = DrawingBoard.Utils.pixelAt(b, parseInt(a.coords.x, 10), parseInt(a.coords.y, 10)),
            l = k[f],
            m = this.opts.fillTolerance;
        if (!DrawingBoard.Utils.compareColors(l, DrawingBoard.Utils.RGBToInt(h, i, j), m)) {
            for (var n, o = [k], p = b.width - 1, q = b.height - 1; n = o.pop();) DrawingBoard.Utils.compareColors(n[f], l, m) && (b.data[n[c]] = h, b.data[n[c] + 1] = i, b.data[n[c] + 2] = j, n[d] > 0 && o.push(DrawingBoard.Utils.pixelAt(b, n[d] - 1, n[e])), n[d] < p && o.push(DrawingBoard.Utils.pixelAt(b, n[d] + 1, n[e])), n[e] > 0 && o.push(DrawingBoard.Utils.pixelAt(b, n[d], n[e] - 1)), n[e] < q && o.push(DrawingBoard.Utils.pixelAt(b, n[d], n[e] + 1)));
    ...
    draw: function () {
        ...
        } else this.dom.$cursor.addClass("drawing-board-utils-hidden"); if (this.isDrawing) {
    ...
    _getInputCoords: function (a) {
        a = a.originalEvent ? a.originalEvent : a;
        var b, c;
        return a.touches && 1 == a.touches.length ? (b = a.touches[0].pageX, c = a.touches[0].pageY) : (b = a.pageX, c = a.pageY), {
    ...
    initialize: function () {
        ...
        if (this.opts.back && (a += '<button class="drawing-board-control-navigation-back">&larr;</button>'), this.opts.forward && (a += '<button class="drawing-board-control-navigation-forward">&rarr;</button>'), this.opts.reset && (a += '<button class="drawing-board-control-navigation-reset">&times;</button>'), this.$el.append(a), this.opts.back) {
            ...
            this.board.ev.bind("historyNavigation", $.proxy(function (a) {
                1 === a ? b.attr("disabled", "disabled") : b.removeAttr("disabled")
            }, this)), this.$el.on("click", ".drawing-board-control-navigation-back", $.proxy(function (a) {
        ...
        if (this.opts.forward) {
            ...
            this.board.ev.bind("historyNavigation", $.proxy(function (a) {
                a === this.board.history.values.length ? c.attr("disabled", "disabled") : c.removeAttr("disabled")
            }, this)), this.$el.on("click", ".drawing-board-control-navigation-forward", $.proxy(function (a) {
